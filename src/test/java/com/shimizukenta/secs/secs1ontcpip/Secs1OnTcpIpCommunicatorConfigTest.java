...
class Secs1OnTcpIpCommunicatorConfigTest {
	
	@Test
	@DisplayName("Secs1OnTcpIpCommunicatorConfig default")
	void testDefault() {
		...
		// reconnectSeconds
		assertEqualsTimeoutAndUnit(config.reconnectSeconds().get(), 5000, TimeUnit.MILLISECONDS);
	...
	@Test
	@DisplayName("Secs1OnTcpIpCommunicatorConfig set-and-get")
	void testSetAndGet() {
		Secs1OnTcpIpCommunicatorConfig config = new Secs1OnTcpIpCommunicatorConfig();
		...
		// reconnectSeconds
		config.reconnectSeconds(10.0F);
		assertEqualsTimeoutAndUnit(config.reconnectSeconds().get(), 10000, TimeUnit.MILLISECONDS);
